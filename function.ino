...
uint8_t picArray[240][320];
const int CS = 5;
const int lightThresh = 20;
const int numRows = 240;
const int numCols = 320;
const int intensityT = 1;
bool is_header = false;
int mode = 0;
uint8_t start_capture = 0;
ArduCAM myCAM( OV5642, CS );
uint8_t read_fifo_burst(ArduCAM myCAM);
void findCentroids(uint8_t picArray[][numCols]);
void checkNeighbors(int row, int col, uint8_t picArray[][numCols], int* numCells, int* cX, int* cY);
  ...
  if (Serial.available())
  {
    ...
    switch (temp)
    {
      ...
      case 0x32:
        ...
        for (i = 0; i < numRows; i++) {
          for (j = 0; j < numCols; j++) {
            Serial.print(picArray[i][j], DEC);
          }
          Serial.println();
        }; break;
      case 0x33:
        findCentroids(picArray); break;
      case 0x40:
    ...
    if (myCAM.get_bit(ARDUCHIP_TRIG, CAP_DONE_MASK))
    {
      ...
      for (i = 0; i < 240; i++)
      {
        for (j = 0; j < 320; j++)
        {
          ...
          Serial.write(VH);
          delayMicroseconds(12);
          rgb = ((uint16_t)VH << 8) | (uint16_t)VL;
          red = 100 * (rgb >> 11) / 31;
          green = 100 * ((rgb >> 5) & 0x3F) / 63;
          blue = 100 * (rgb & 0x1F) / 31;
          uint8_t lum = (0.2126 * red) + (0.7152 * green) + (0.0722 * blue);
          if (lum < lightThresh) {
            picArray[i][j] = 1;
          }
        }
...
void findCentroids(uint8_t picArray[][numCols]) {
  ...
  for (i = 0; i < numRows; i++) {
    for (j = 0; j < numCols; j++) {
      ...
      if (picArray[i][j] == 1) {
        ...
        int cY = 0;
        bool allOne = true;
        for (x = i - intensityT; x < i + intensityT + 1; x++) {
          for (y = j - intensityT; y < j + intensityT + 1; y++) {
        ...
        if (allOne) {
          checkNeighbors(i, j, picArray, &numCells, &cX, &cY);
          cX = cX / numCells;
          cY = cY / numCells;
          Serial.print("x: ");
          Serial.print(cX, DEC);
  ...
  if (!hasCentroid) {
    Serial.println("There is no centroid.");
  }
  return;
}
...
void checkNeighbors(int row, int col, uint8_t picArray[][numCols], int* numCells, int* cX, int* cY) {
  if (picArray[row][col] == 1) {
    if (outOfBounds(row, col, numRows, numCols)) {
      return;
    }
    ...
    *cY += row;
    ...
    //    if(picArray[row-1][col-1] == 1){
    //      checkNeighbors(row-1, col-1, picArray, numCells, cX, cY);
    //    }
    if (picArray[row - 1][col] == 1) {
      checkNeighbors(row - 1, col, picArray, numCells, cX, cY);
    }
    //    if(picArray[row-1][col+1] == 1){
    //      checkNeighbors(row-1, col+1, picArray, numCells, cX, cY);
    //    }
    if (picArray[row][col - 1] == 1) {
      checkNeighbors(row, col - 1, picArray, numCells, cX, cY);
    }
    if (picArray[row][col + 1] == 1) {
      checkNeighbors(row, col + 1, picArray, numCells, cX, cY);
    }
